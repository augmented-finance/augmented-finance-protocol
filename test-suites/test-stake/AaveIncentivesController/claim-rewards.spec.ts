...
makeSuite('AaveIncentivesController claimRewards tests', (testEnv) => {
  for (const {
    ...
    it(caseName, async () => {
      ...
      const { aaveIncentivesController, stakedAaveV2, aaveToken, aDaiMock } = testEnv;
      ...
      const destinationAddressBalanceBefore = await (toStake ? stakedAaveV2 : aaveToken).balanceOf(
        destinationAddress
      );
      ...
      const destinationAddressBalanceAfter = await (toStake ? stakedAaveV2 : aaveToken).balanceOf(
        destinationAddress
      );

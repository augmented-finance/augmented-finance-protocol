...
export const deployStakedAaveV2 = async (
  ...
  const id = eContractid.StakedAgfV1;
  const args: string[] = [
    stakedToken,
    rewardsToken,
    cooldownSeconds,
  ...
  const instance = await deployContract<StakedAgfV1>(id, args);
  if (verify) {
...
export const getStakedAaveV2 = getContractFactory<StakedAgfV1>(eContractid.StakedAgfV1);
